package com.images.sex.entity;

import java.io.Serializable;

import com.images.sex.entity.enums.ImgStatus;

public class ImgVier implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private Integer id;
	private String nameVier;
	private String sourceAvate;
	private String description;
	private ImgStatus imgStatus;
	private ImgThumbnai thumbnai;
	private ImgLibrary library;
	private String dateCreate;
	private Double money;
	private String nameAvata;
	private String nameUUID;
	
	public ImgVier() {
		super();
	}

	public ImgVier(Integer id, String nameVier, String sourceAvate,
			String description, ImgStatus imgStatus, ImgThumbnai thumbnai,
			ImgLibrary library, String dateCreate, Double money,
			String nameAvata, String nameUUID) {
		super();
		this.id = id;
		this.nameVier = nameVier;
		this.sourceAvate = sourceAvate;
		this.description = description;
		this.imgStatus = imgStatus;
		this.thumbnai = thumbnai;
		this.library = library;
		this.dateCreate = dateCreate;
		this.money = money;
		this.nameAvata = nameAvata;
		this.nameUUID = nameUUID;
	}

	public Integer getId() {
		return id;
	}

	public void setId(Integer id) {
		this.id = id;
	}

	public String getNameVier() {
		return nameVier;
	}

	public void setNameVier(String nameVier) {
		this.nameVier = nameVier;
	}

	public String getSourceAvate() {
		return sourceAvate;
	}

	public void setSourceAvate(String sourceAvate) {
		this.sourceAvate = sourceAvate;
	}

	public String getDescription() {
		return description;
	}

	public void setDescription(String description) {
		this.description = description;
	}

	public ImgStatus getImgStatus() {
		return imgStatus;
	}

	public void setImgStatus(ImgStatus imgStatus) {
		this.imgStatus = imgStatus;
	}

	public ImgThumbnai getThumbnai() {
		return thumbnai;
	}

	public void setThumbnai(ImgThumbnai thumbnai) {
		this.thumbnai = thumbnai;
	}

	public ImgLibrary getLibrary() {
		return library;
	}

	public void setLibrary(ImgLibrary library) {
		this.library = library;
	}

	public String getDateCreate() {
		return dateCreate;
	}

	public void setDateCreate(String dateCreate) {
		this.dateCreate = dateCreate;
	}

	public Double getMoney() {
		return money;
	}

	public void setMoney(Double money) {
		this.money = money;
	}

	public String getNameAvata() {
		return nameAvata;
	}

	public void setNameAvata(String nameAvata) {
		this.nameAvata = nameAvata;
	}

	public String getNameUUID() {
		return nameUUID;
	}

	public void setNameUUID(String nameUUID) {
		this.nameUUID = nameUUID;
	}

	public static long getSerialversionuid() {
		return serialVersionUID;
	}
	
}
